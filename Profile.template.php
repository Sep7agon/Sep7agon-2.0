...
<div id="profileview" class="flow_auto">
	...
	<div id="basicinfo">
		<div class="windowbg">
			...
			<div class="content flow_auto">
				<div class="username"><h4>', $context['member']['name'], '</h4></div>
	...
	echo '
					...
					<a href="', $scripturl, '?action=profile;area=statistics;u=', $context['id_member'], '">', $txt['statPanel'], '</a><br />
					<a href="#" class="showLikeNotification" onclick="lpObj.showLikeNotification()">Show Likes</a>
				</p>';
...
function template_groupMembership()
{
	...
	echo '
		<form action="', $scripturl, '?action=profile;area=groupmembership;save" method="post" accept-charset="', $context['character_set'], '" name="creator" id="creator">
			...
			<p class="description">In this section of your profile you can change how your posts are displayed.</p>';
	...
	if (!empty($context['group_request']))
	{
		echo '
			<div class="groupmembership">
				<div class="cat_bar">
					<h3 class="catbg">Request Nameplate</h3>
				</div>
				<span class="upperframe"><span></span></span>
	...
	else
	{
		echo '
			<table border="0" width="100%" cellspacing="0" cellpadding="4" class="table_grid">
				<thead>
					<tr class="catbg">
						<th class="first_th" scope="col" ', $context['can_edit_primary'] ? ' colspan="2"' : '', '>Your Nameplates</th>
						<th class="last_th" scope="col"></th>
					</tr>
				</thead>
				<tbody>';
		...
		if ($context['can_edit_primary'])
			...
			<div class="padding righttext">
				<input type="submit" value="Change Nameplate" class="button_submit" />
			</div>';
		...
		if (!empty($context['groups']['available']))
		{
			...
			foreach ($context['groups']['available'] as $group)
			{
				...
				if ($group['type'] == 3)
					echo '
							<a href="', $scripturl, '?action=profile;save;u=', $context['id_member'], ';area=groupmembership;', $context['session_var'], '=', $context['session_id'], ';gid=', $group['id'], '">Get Nameplate</a>';
				elseif ($group['type'] == 2 && $group['pending'])
					echo '
							', $txt['approval_pending'];
				elseif ($group['type'] == 2)
					echo '
							<a href="', $scripturl, '?action=profile;u=', $context['id_member'], ';area=groupmembership;request=', $group['id'], '">Request Nameplate</a>';
